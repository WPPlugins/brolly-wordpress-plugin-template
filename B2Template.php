...
class B2Template
{
	...
	*/
	...
	function B2Template()
	{
		...
		*/
		...
		/* You can attach as many functions as you want to a single action and Wordpress will run them in sequence.  If you want them to run in a different order then you add them, you can define the third parameter which indicated the priority. So my RouteActions function above will run after the SetLanguage action here, because the priority of SetLanguage is 1 (first priority) and RouteActions is 2 (second priority) the SetLanguage function sets up our plugins internationalization. More on this later! */
		...
		add_action('template_redirect',array($this,'StylesAndScripts'));
	...
	function Activate() 
	{
		...
		/* This is the current version of the plugin table */
		$db_version = 4.0;
		...
		$installed_ver = (float) get_option( 'B2Template_db_version' );
		
		/* If $installed_ver is empty, then it's likely that the user is activating the plugin for the first time, so we want to create or database tables. */
		if(empty($installed_ver))
		{
			/* Here we double check to see if the table we are going to create already exists. If it does, we don't want overwrite it. */
			...
			/* Now we create the option that will tell us in the future, what version of the plugin database structure is installed. The add_option function let's us quickly insert a value into the WordPress options table.*/
			add_option( 'B2Template_db_version', $db_version );
			
		}		
		else { /* If $installed_ver is set, then we know we have an outdated version of the tables installed, so we need to upgrade them. */
		
			if( $installed_ver < 2.0)
			{
				 /* From version 2.0 to version 3.0 I renamed a field called fieldOne to find, and added a new column called replace */
			...
			update_option( 'B2Template_db_version', $db_version );
	...
	function InsertAdminMenuLink() {
		...
		add_action( 'admin_print_scripts-'.$page , array( $this, 'AdminStylesAndScripts' ) );
	...
	/*****
		...
		*****/
	function AdminStylesAndScripts()
	{
		...
		wp_localize_script(
			...
			array( // This is an array of variables that we want to send to our Javascript file. The first two are nonce security keys that we'll use in our AJAX functions to make sure the request are legit.
				'DeleteFindAndReplaceWord'=>wp_create_nonce('DeleteFindAndReplaceWord'),
				'AddFindAndReplaceWord'=>wp_create_nonce('AddFindAndReplaceWord'),
	...
	/*****
	6.2 Action to add JavaScript and CSS to viewer facing pages
	This function adds JavaScript and CSS to the viewer facing side of WordPress. Scroll up to AdminStylesAndScripts() to see how these functions work 
		*****/
	function StylesAndScripts()
	{
		...
		wp_localize_script('B2Template_SubmitForm','B2Template',
			array(
				'AddFactCheck'=>wp_create_nonce('AddFactCheck'),
				'AjaxError'=>__('An error occurred. Please try your action again. If the problem persists please contact the plugin developer.','B2Template')
			));
	...
	*****/
	...
	function RouteActions()
	{
		...
		$ajax = false; // By default we assume a non Ajax action
		$action = false; // By default we assume no Action taken place
		
		if(isset($_POST['B2Template_Ajax']) != false) // If the POST variable B2Template_Ajax is set, then it is an ajax action that we want to perform
		{
			$action = $_POST['B2Template_Ajax'];
			$ajax = true;
		}
		elseif(isset($_GET['B2Template_Action'])) // Otherwise it was a form submit
		{
			$action =$_GET['B2Template_Action'];
		}
		...
		if($ajax)
		{
			return $this->AjaxResponse($action);
		}
	...
	function AddFindAndReplaceWord($find, $replace) {
		...
		if(empty($find)) // If the $find parameter is empty, we can't save the rule, so report an error and return false
		{
			...
			$this->errors->add(
				ERROR_FIND_EMPTY, // This is the error code 0, see the class constant definition at the top of the plugin
		...
		*/
		...
		{
			$this->errors->add(ERROR_DUPLICATE_RULE, sprintf(__('A replace rule for that term already exists. Delete the existing rule for "%s" and try again, or create a different rule.','B2Template'),stripslashes($find)));
			return false;
		}
		...
		{
			$this->errors->add(ERROR_MYSQL, __('There was a problem updating the database. Try your action again, and if it continues to fail contact the plugin developer', 'B2Template'));
		}
	...
	function AddFactCheck($post_id, $fact, $comment, $source) {
		...
		{
			$this->errors->add(ERROR_MISSING_FIELDS,__('All fields are required, please fill in the missing fields and submit your fact check again.','B2Template')); 
			return false;
		}
